...
void invulnerable(int timer, int *coinsCounter,int *imortal, int *timer2);
...
int main()
{
    ...
    do
    {
        ...
        if(tela==1)//Condicionar que vale se o jogo tiver começado.
        {
            ...
            invulnerable(timer, &coinsCounter, &imortal, &timer2);
...
void menuIn(int *tela, int *hpN)
{
    int i, n, option=1;
    char dir;
    char text[7][80]={{"A Espada Perdida de Midas"},{"Começar"},{"Carregar"},{"Níveis"},{"Opções"},{"Sair"},("Versão: 0.6.0")};
